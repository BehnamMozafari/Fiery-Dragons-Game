...
/**
 * The LoadSquareBoardStrategy class implements the SetBoardStrategy interface and provides the
 * setup for displaying chit cards and volcano cards on the game board.
 */
public class LoadSquareBoardStrategy implements SetBoardStrategy {
  ...
  public void setUpBoard(GameBoard board) {
    ...
    List<ChitCard> flippedCard = GameEngine.getInstance().getFlippedChitCards();

    for (ChitCard card : flippedCard) {
      card.flip();
    }
  }
   ...
   */
  public void setUpChitCardPanel(GameBoard board) {
  ...
  private void setUpVolcanoCardBorder(GameBoard board) {
    ...
    JPanel eastPanel = createBorderPanel(cardsPerSide + (remainingCards > 1 ? 1 : 0), 1);
    JPanel southPanel = createBorderPanel(1, cardsPerSide + (remainingCards > 2 ? 1 : 0));
  ...
  /**
   * Adds empty spaces to the north and south panels.
   *
   * @param northPanel The north panel.
   * @param southPanel The south panel.
   */
  private void addEmptySpaces(JPanel northPanel, JPanel southPanel) {
    northPanel.add(createEmptySpace());
    southPanel.add(createEmptySpace());
  }

  /**
   * Creates an empty space panel.
   *
   * @return The created JPanel.
   */
  private JPanel createEmptySpace() {
    JPanel emptySpace = new JPanel();
    emptySpace.setPreferredSize(EMPTY_SPACE_SIZE);
    return emptySpace;
  }

  /**
   * Adds volcano cards to the specified panels.
   *
   * @param volcanoCards The list of volcano cards to add.
   * @param northPanel   The north panel.
   * @param southPanel   The south panel.
   * @param eastPanel    The east panel.
   * @param westPanel    The west panel.
   */
  private void addVolcanoCardsToPanels(List<VolcanoCard> volcanoCards, JPanel northPanel,
      JPanel southPanel, JPanel eastPanel, JPanel westPanel) {
    for (int i = 0; i < 2; i++) {
      northPanel.add(setSize(volcanoCards.get(i), VOLCANO_CARD_SIZE));
      southPanel.add(setSize(volcanoCards.get(5 - i), VOLCANO_CARD_SIZE));
    }

    eastPanel.add(setSize(volcanoCards.get(2), VOLCANO_CARD_SIZE));
    eastPanel.add(setSize(volcanoCards.get(3), VOLCANO_CARD_SIZE));
    westPanel.add(setSize(volcanoCards.get(7), VOLCANO_CARD_SIZE));
    westPanel.add(setSize(volcanoCards.get(6), VOLCANO_CARD_SIZE));
  }
